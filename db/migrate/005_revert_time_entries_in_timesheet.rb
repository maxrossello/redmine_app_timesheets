...
class RevertTimeEntriesInTimesheet < ActiveRecord::Migration
  def up
    ...
    case adapter_type
      ...
      when :sqlite
        sql = "REPLACE INTO time_entries (fixed_version_id) SELECT issues.fixed_version_id FROM issues INNER JOIN time_entries ON issues.id = time_entries.issue_id WHERE time_entries.in_timesheet = 't' AND (time_entries.fixed_version_id is NULL);"
        connection.update(sql)
      else
